            ...
            <div className="row">
              <div className="col-md-6 image">
                {images.map((image, index) => (
                  <div key={index} className={`cen-img fade-img ${currentImage === index ? 'show' : ''}`} style={{ transition: 'opacity 1s ease-in-out', opacity: currentImage === index ? 1 : 0 }}>
                    <Image
                      src={image.src}
                      alt={image.alt}
                      layout="intrinsic"
                      width={800} // Specify a width
